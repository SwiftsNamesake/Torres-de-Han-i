...
main = do
	...
	where
		...
		render markers dt 	= pictures $ [spiral, dotGrid 20 20 $ (8+) . (16*) . abs . sin $ 1/8*dt]  --[outline markers, silhouette markers, dots markers 0 0, dots markers dx 0]
		...
		dotGrid rws cls pad = pictures $ grid (\ r c -> color (makeColor (sin (r*pad*1/8)) (cos (c*pad*1/8)) 0.86 1.0)
																. translate (c*pad) (r*pad) $ circleSolid (5 + 3 * (sin $ c*pad))) rws cls
		spiral = pictures [rotate (n*5) . translate (n*2) 0 $ circleSolid 5 | n <- [1..320]]
